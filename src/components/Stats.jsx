...
const Status = () => {
  ...
  const coffeinLevel = calculateCurrentCaffeineLevel(coffeeConsumptionHistory);
  const worningLevel =
    coffeinLevel < statusLevels.low.maxLevel
      ? "low"
      : coffeinLevel < statusLevels.moderate.maxLevel
      ? "moderate"
      : "high";
  return (
    <>
      ...
      <div className="stats-grid">
        <StatCard lg title="Active Caffeine Level">
          <div className="status">
            <p>
              <span className="stat-text">{coffeinLevel}</span>mg
            </p>
            <h5
              style={{
                color: statusLevels[worningLevel].color,
                backgroundColor: statusLevels[worningLevel].background,
              }}
            >
              {worningLevel}
            </h5>
